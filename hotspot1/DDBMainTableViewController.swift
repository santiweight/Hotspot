...
class DDBMainTableViewController: UITableViewController {
        ...
        for i in 0..<25 {
            for j in 0..<2 {
                ...
                if j == 0 {
                    ...
                    tableRow?.EventTitle = gameTitleArray[c]
                } else {
                    tableRow?.EventTitle = "Comet Quest"
                }
                tableRow?.Latitude = Int(arc4random_uniform(3000)) as NSNumber?
                tableRow?.Longitude = Int(arc4random_uniform(100)) as NSNumber?
                tableRow?.CurrentPeople = Int(arc4random_uniform(100)) as NSNumber?
                tableRow?.ExpectedPeople = Int(arc4random_uniform(100)) as NSNumber?
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let myTableRows = self.tableRows {
            ...
            cell.textLabel?.text = "ID: \(item.EventId!), Title: \(item.EventTitle!)"

            if let myDetailTextLabel = cell.detailTextLabel {
                myDetailTextLabel.text = "Latitude:\(item.Latitude!), Longitude:\(item.Longitude!), Expected People:\(item.ExpectedPeople!                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           ), Current People:\(item.CurrentPeople!)"
            }
